...
const GameBoard: React.FC<GameBoardProps> = ({ gameState, onLevelCompleteClick, replayMode }) => {
  ...
  return (
    ...
    >
      {/* Background image layer (supports special level filter) */}
      <div
        ...
        style={{
          ...
          filter: specialLevel?.bgFilter || 'none',
          zIndex: 0,
        }}
      />

      {/* Special level tint overlay */}
      {specialLevel?.bgTint && (
        <div
          className="absolute inset-0 pointer-events-none"
          style={{
            backgroundColor: specialLevel.bgTint,
            zIndex: 1,
          }}
        />
      )}
        ...
        return (
          <div className="absolute inset-0 flex items-center justify-center z-[55] pointer-events-none">
            <div className={`bg-black bg-opacity-70 text-white rounded-xl px-6 py-4 sm:px-10 sm:py-6 text-center ${
              announcementSpecial ? 'border-2 border-yellow-400' : ''
            }`}>
              <h2 className="text-2xl sm:text-4xl font-bold">
                Level {gameState.levelAnnouncement.level}
                {announcementSpecial && (
                  <span className="block text-lg sm:text-2xl text-yellow-300 mt-1">
                    {announcementSpecial.name}
                  </span>
                )}
              </h2>
              <p className="text-sm sm:text-lg mt-1 text-gray-300">
                {announcementSpecial?.announcementSubtitle
